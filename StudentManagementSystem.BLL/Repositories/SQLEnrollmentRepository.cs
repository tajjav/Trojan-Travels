...
namespace StudentManagementSystem.BLL.Repositories
{
    public class SQLEnrollmentRepository : IEnrollmentRepository
    {
        ...
        public async Task<IList<CourseEnrollmentCount>> CourseEnrollmentCount(string title)
        {
           ...
           return await query.GroupBy(e => e.Course.Title)
                    .Select(c => new CourseEnrollmentCount()
                    {
                        CourseTitle = c.Key,
        ...
        public async Task<Enrollment> GetEnrollment(int id)
        {
            return await _context.Enrollments
                .Include(s => s.Student)
                .Include(c => c.Course).FirstOrDefaultAsync(x => x.Id == id);
        }
        ...
        public async Task<Enrollment> UpdateEnrollment(Enrollment updatedEnrollment)
        {
            Enrollment enrollment = await _context.Enrollments
                .FirstOrDefaultAsync(x => x.Id == updatedEnrollment.Id);
